...
exports.getBookingsByGuestId = async (req, res) => {
  try {
    ...
    const bookings = await Booking.find({ guestId: id }).populate("cabinId");
    res.status(200).json({
      status: true,
      bookings,
    });
  } catch (err) {
    return errorResponse(res, "Error retrieving guest's bookings", 500);
  }
};
