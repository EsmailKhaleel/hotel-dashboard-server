...
exports.validateGuestBody = (body) => {
  ...
  // Validate full name (basic format check + length between 2 and 50 check)
  const fullNameRegex = /^[A-Za-z\s]{2,50}$/;
  if (!fullNameRegex.test(body.fullName)) {
    return {
      isValid: false,
      error:
        "Invalid full name format. Must be 2-50 characters long and contain only letters and spaces",
    };
  }
  ...
  if (body.nationality) {
    if (typeof body.nationality !== "string" || body.nationality.trim().length < 2) {
      return {
        isValid: false,
        error: "Invalid nationality format. Must be at least 2 characters long."
      };
    }
  }

  // Validate countryFlag if present
  if (body.countryFlag) {
    if (typeof body.countryFlag !== "string" || body.countryFlag.trim().length < 2) {
      return {
        isValid: false,
        error: "Invalid country flag format. Must be at least 2 characters long."
      };
    }
  }
  ...
  if (body.nationalID) {
    const nationalIdRegex = /^[A-Z0-9]{8,12}$/;
    if (!nationalIdRegex.test(body.nationalID)) {
      return {
        isValid: false,
        error:
          "Invalid national ID format. Must be at least 8 digits and no more than 12 digits",
      };
    }
  }
  ...
  if (body.phoneNumber) {
    ...
    if (!phoneRegex.test(body.phoneNumber)) {
      return {
        isValid: false,
        error:
          "Invalid phone number format. Must be at least 8 digits and may include +, spaces, or hyphens",
      };
    }
  }
  ...
  if (body.address) {
    if (typeof body.address !== "string" || body.address.trim().length < 2) {
      return {
        isValid: false,
        error: "Invalid address format. Must be at least 2 characters long."
      };
    }
  }
